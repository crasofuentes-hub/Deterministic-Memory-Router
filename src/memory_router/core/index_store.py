...
class InMemoryAgentIndex:
    ...
    def add(self, text: str, ts_unix: int, meta: Dict[str, Any] | None = None) -> MemoryItem:
        if meta is None:
            meta = {}
        item = MemoryItem(agent=self.agent, stable_id=self._next_id, text=text, ts_unix=int(ts_unix), meta=dict(meta))
