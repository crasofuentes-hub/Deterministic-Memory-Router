...
class FaissHNSWHotIndex:
    ...
    def __post_init__(self) -> None:
        ...
        try: faiss.omp_set_num_threads(int(self.omp_threads))
    ...
    def _load_or_create(self, user_key: str) -> faiss.Index:
        if user_key in self._idx: return self._idx[user_key]
        p = self._path(user_key)
        if os.path.exists(p):
            idx = faiss.read_index(p)
            try: idx.hnsw.efSearch = int(self.ef_search)
            except Exception: pass
        else:
            idx = self._create()
    ...
    def add(self, user_key: str, vec: np.ndarray, persist: bool = False) -> int:
        with self._lock:
            ...
            if v.shape[1] != self.dim: raise ValueError(f"dim mismatch: expected {self.dim}, got {v.shape[1]}")
            before = int(idx.ntotal)
            idx.add(v)
            if persist: faiss.write_index(idx, self._path(user_key))
            return before
